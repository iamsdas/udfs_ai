@fused.udf
def udf(
    bounds: fused.types.Bounds = [-0.11047509709158529,51.51225013101162,-0.10604030594695288,51.51490454478444],
    ...
    # Move key cols after 'id', geometry last
    first = [c for c in ['id', 'height', 'class', 'subtype', 'num_floors'] if c in gdf.columns]
    rest = [c for c in gdf.columns if c not in first and c != 'geometry']
    gdf = gdf[first + rest + ['geometry']]
